<?php
/**
 * Members and methods for WTG Tasks Manager views that use post boxes (eventually all views)
 *
...
/**
 * WTG Tasks Manager Base View class
 * 
 * Taking from WTGTASKSMANAGER plugin and customized to pave the way for postboxes on multiple
 ...
 * @version 1.0.2
 */
abstract class WTGTASKSMANAGER_View {
     ...
     * @since 0.0.1
     *
     * @var WTGTASKSMANAGER_Admin_Page
     */
        ...
        if( isset( $this->help_array[ $page_name ] ) ){
            ...
            if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pageabout' ] ) ){
                ...
                $screen->add_help_tab( array(
                    'id' => $page_name,
                    'title' => __( 'About', 'wtgtasksmanager' ) . ' ' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagetitle' ],
                ...
                <p><strong>' . __( 'For more information:', 'wtgtasksmanager' ) . '</strong></p>' . $pagereadmeurl . $pagevideourl . $pagediscussionurl . $pagefaqurl );
    ...
    */
    public function setup( $action, array $data ) {
        ...
        $this->WTGTASKSMANAGER->enqueue_script( 'common', array( 'jquery', 'postbox' ), array() );
    ...
    */
    protected function add_text_box( $id, $callback, $context = 'normal', $wrap = false ) {
        ...
        $long_id = "wtgtasksmanager_{$this->action}-{$id}";
    ...
    */
    protected function add_meta_box( $id, $title, $callback, $context = 'normal', $priority = 'default', $callback_args = null ) {
        ...
        $id = "wtgtasksmanager_{$this->action}-{$id}";         
    ...
    */
    protected function action_nonce_field( array $data, array $box ) {
        wp_nonce_field( $this->WTGTASKSMANAGER->nonce( $this->action ) ); echo "\n";
    }
    ...
    */
    protected function action_field( array $data, array $box ) {
        echo "<input type=\"hidden\" name=\"action\" value=\"wtgtasksmanager_{$this->action}\" />\n";
    }
    ...
    */
    protected function textbox_submit_button( array $data, array $box ) {
        $caption = isset( $data['submit_button_caption'] ) ? $data['submit_button_caption'] : __( 'Save Changes', 'wtgtasksmanager' );
        ?>
