...
* @version 1.0 
*/
class WTGTASKSMANAGER_UI extends WTGTASKSMANAGER {     
    ...
    public function __construct() {
        ...
        $this->WPCore = self::load_class( 'WTGTASKSMANAGER_WPCore', 'class-wpcore.php', 'classes' );  
    }  
          
    /**
    * Called by main WTGTASKSMANAGER() class using proper hook
    * 
    ...
    */
    public function add_dashboard_widgets() {
        ...
        wp_add_dashboard_widget(
             ...
             __( 'Test Widget', 'wtgtasksmanager' ),// Title.
    ...
    */
    ...
    /**
    * Displays a table of wtgtasksmanager_option records with ability to view their value or delete them
    ...
    */
    public function list_optionrecordtrace( $form = false, $size = 'Small', $optiontype = 'all' ){ 
        // first get all records that begin with wtgtasksmanager_
        $wtgtasksmanagerrecords_result = $this->DB->options_beginning_with( 'wtgtasksmanager_' );
        $counter = 1;?>
            
            <script language="JavaScript">
            function wtgtasksmanager_deleteoptions_checkboxtoggle(source) {
              checkboxes = document.getElementsByName( 'wtgtasksmanager_deleteoptions_array[]' );
        ...
        if( $form){
            echo '<input type="checkbox" onClick="wtgtasksmanager_deleteoptions_checkboxtoggle(this)" /> '. __( 'Select All Options', 'wtgtasksmanager' ) .'<br/>';
        }
        
        $html = '';
                        
        foreach( $wtgtasksmanagerrecords_result as $key => $option ){
            
            if( $form){
                $html = '<input type="checkbox" name="wtgtasksmanager_deleteoptions_array[]" value="'.$option.'" />';
            }
        ...
        <div class="wtgtasksmanager_screenintro_container">
            <div class="welcome-panel">
                ...
                <div class="welcome-panel-content">
                    ...
                    <p>'.__( 'Pledge £9.99 to the WTG Tasks Manager project for 50% discount on the premium edition.' ).'</p>                                                     
                </div>
    ...
    /**
    * Builds a nonced link to any admin page
    * 
    * @package WTG Tasks Manager
    * @author Ryan R. Bayne
    * @since 0.0.1
    ...
    */
    public function linkaction( $page_name_or_post_id, $action, $title = 'WTG Tasks Manager admin link', $text = 'Click Here', $values = '', $class = ' class="button c2pbutton"', $file = 'admin.php' ){
        ...
        $pagepost = 'page';
        if( strstr( $file, 'post.php' ) ) { $pagepost = 'post'; }
        
        $admin_url = admin_url() . $file . '?' . $pagepost . '=' . $page_name_or_post_id . '&wtgtasksmanageraction=' . $action  . $values;
        
        return '<a href="'. wp_nonce_url( $admin_url, $action ) . '" title="' . $title . '"' . $class . '>' . $text . '</a>';
    }
    ...
    */
    public function option_switch( $title, $name, $id, $current_value = 'nocurrent123', $onlabel = 'Enabled', $offlabel = 'Disabled', $default = 'enabled' ){
        ...
        <!-- Option Start -->
        <tr valign="top">
            <th scope="row"><?php _e( $title, 'wtgtasksmanager' ); ?></th>
            <td>
    ...
    * @deprecated use class-forms.php      
    */
    public function option_text( $title, $name, $id, $current_value = '', $readonly = false, $class = 'wtgtasksmanager_inputtext', $append_text = '', $left_field = false, $right_field_content = '', $required = false, $validation = false ){
    ...
    */
    public function option_text_simple( $title, $nameandid, $current_value = '', $required = false, $validation = false ){
        $this->option_text( $title, $nameandid, $nameandid, $current_value, false, 'wtgtasksmanager_inputtext', '', false, '', $required, $validation );
    }
    ...
    */
    public function metabox_icons( $form_id, $trash_icon = false ){
        ...
        if( WTGTASKSMANAGER::is_dashboard() ) {
            return false;    
        }
    ...
    */
    public function postbox_content_header( $form_title, $form_id, $introduction = false, $trash_icon = false, $uploader = false ) {?>
        <p>
            ...
            <?php if( $introduction !== false && !empty( $introduction) ){echo "<p class=\"wtgtasksmanager_boxes_introtext\">". $introduction ."</p>";}?>
    ...
    */          
    public function box_information_icon( $page_name, $view_name, $form_id ) {
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formreadmoreurl' ] ) ){
            $all_help_content .= '<p>';
            $all_help_content .= __( 'You are welcome to visit the', 'wtgtasksmanager' ) . ' ';
            $all_help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formreadmoreurl' ] . '"';
            $all_help_content .= 'title="' . __( 'Visit the WTG Tasks Manager website and read more about', 'wtgtasksmanager' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '"';
            $all_help_content .= 'target="_blank"';
            $all_help_content .= '>';
            $all_help_content .= __( 'WTG Tasks Manager Website', 'wtgtasksmanager' ) . '</a> ' . __( 'to read more about', 'wtgtasksmanager' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ];           
            $all_help_content .= '.</p>';
        }  
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formvideourl' ] ) ){
            $all_help_content .= '<p>';
            $all_help_content .= __( 'There is a', 'wtgtasksmanager' ) . ' ';
            $all_help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formvideourl' ] . '"';
            $all_help_content .= 'title="' . __( 'Go to YouTube and watch a video about', 'wtgtasksmanager' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '"';
            $all_help_content .= 'target="_blank"';
            $all_help_content .= '>';            
            $all_help_content .= __( 'YouTube Video', 'wtgtasksmanager' ) . '</a> ' . __( 'about', 'wtgtasksmanager' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ];           
            $all_help_content .= '.</p>';
        }
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formdiscussurl' ] ) ){
            $all_help_content .= '<p>';
            $all_help_content .= __( 'We invite you to take discuss', 'wtgtasksmanager' ) . ' ';
            $all_help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formdiscussurl' ] . '"';
            $all_help_content .= 'title="' . __( 'Visit the WebTechGlobal forum to discuss', 'wtgtasksmanager' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '"';
            $all_help_content .= 'target="_blank"';
            $all_help_content .= '>';            
            $all_help_content .= $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '</a> ' . __( 'on the WebTechGlobal Forum', 'wtgtasksmanager' );           
            $all_help_content .= '.</p>';
        }        
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'options' ] ) ){
            foreach( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'options' ] as $option_id => $option_array ){
                ...
                if( isset( $option_array['optionurl'] ) ){
                    ...
                    $all_help_content .= ' title="' . __( 'Read More about', 'wtgtasksmanager' )  . ' ' . $option_array['optiontitle'] . '"';
                    $all_help_content .= ' target="_blank">';
                    $all_help_content .= __( 'Read More', 'wtgtasksmanager' ) . '</a>';      
                }
      
                if( isset( $option_array['optionvideourl'] ) ){
                    $all_help_content .= ' - <a href="' . $option_array['optionvideourl'] . '"';
                    $all_help_content .= ' title="' . __( 'Watch a video about', 'wtgtasksmanager' )  . ' ' . $option_array['optiontitle'] . '"';
                    $all_help_content .= ' target="_blank">';
                    $all_help_content .= __( 'Video', 'wtgtasksmanager' ) . '</a>';      
                }
    ...
    * 
    * @deprecated use wtgtasksmanager_n_postresult as of 13th February 2013
    */
    ...
    /**
    * New Notice Function (replacing WTGTASKSMANAGER_Notice::notice_depreciated() )
    ...
    */
    public function update_persistentnotifications_array( $notifications_array ){
        return update_option( 'wtgtasksmanager_notifications',maybe_serialize( $notifications_array ) );    
    }
    ...
    */
    public function notice_depreciated( $message, $type = 'success', $size = 'Extra', $title = false, $helpurl = 'www.webtechglobal.co.uk', $output_type = 'echo', $persistent = false, $clickable = false, $user_type = false ){
        ...
        if( is_admin() || $output_type == 'public' ){
            ...
            if( $helpurl != false && $helpurl != '' && $helpurl != 'http://www.wtgtasksmanager.com/support' ){$clickable = false;}
    ...
    */
    ...
    public function add_admin_footer_text() {
        // show admin footer message (only on pages of WTG Tasks Manager)
        add_filter( 'admin_footer_text', array( $this, '_admin_footer_text' ) );
    }
    
    /**
     * Add a WTG Tasks Manager "Thank You" message to the admin footer content
     *
     * @since 0.0.1
     *
    ...
    */
    public function register_input_validation( $input_title, $input_name, $input_id, $input_validation ) {
        ...
        $form_val_arr = get_option( 'wtgtasksmanager_formvalidation' );
        ...
        update_option( 'wtgtasksmanager_formvalidation', $form_val_arr );
    }
    ...
    */
    public function metabox_nocurrentproject() {
        ...
        the one that was active.', 'wtgtasksmanager' );
        echo '</p>';
    }
...
* @version 1.0
*/
class WTGTASKSMANAGER_Pointers {
