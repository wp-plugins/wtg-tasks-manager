...
* @author Ryan Bayne 
*/                                                 
//class WTGTASKSMANAGER_Files extends WTGTASKSMANAGER { commented 14092014
class WTGTASKSMANAGER_Files { 
    
    public function __construct() {
        $this->PHP = WTGTASKSMANAGER::load_class( 'WTGTASKSMANAGER_PHP', 'class-phplibrary.php', 'classes' ); # php library by Ryan R. Bayne  
    }
    ...
    */
    public function upload_error( $file_error ) {
        $upload_error_strings = array( false,
            __( "The uploaded file exceeds the <code>upload_max_filesize</code> directive in <code>php.ini</code>.", 'wtgtasksmanager' ),
            __( "The uploaded file exceeds the <em>MAX_FILE_SIZE</em> directive that was specified in the HTML form.", 'wtgtasksmanager' ),
            __( "The uploaded file was only partially uploaded.", 'wtgtasksmanager' ),
            __( "No file was uploaded.", 'wtgtasksmanager' ),
            __( "Missing a temporary folder.", 'wtgtasksmanager' ),
            __( "Failed to write file to disk.", 'wtgtasksmanager' ) );  
    ...
    */
    public function singlefile_uploader( $file_array, $uploads = false, $test_size = true, $test_type = false, $overwrite = false ) {
        $result_array = array( 'outcome' => false,
                               'error' => false, 
                               'message' => __( 'The file was uploaded.', 'wtgtasksmanager' ) );
        ...
        if ( $test_size && !( $file_array['size'] > 0 ) ) {
            $result_array['outcome'] = false;
            $result_array['error'] = true;
            $result_array['message'] = __( 'Your file is empty. Empty files are not permitted in this operation.', 'wtgtasksmanager' );
            return $result_array;            
        }
        ...
        if (! @ is_uploaded_file( $file_array['tmp_name'] ) ) {
            $result_array['outcome'] = false;
            $result_array['error'] = true;
            $result_array['message'] = __( 'Sorry your file upload failed. A temporary file could not be created on the server. This may be a temporary problem, please try again.', 'wtgtasksmanager' );
            return $result_array;                    
        }
        ...
        if ( $test_type ) {
            ...
            if ( ( !$type || !$ext ) && !current_user_can( 'unfiltered_upload' ) ) {
                $result_array['outcome'] = false;
                $result_array['error'] = true;
                $result_array['message'] = __( 'Your file does not meet security requirements and has not been uploaded.', 'wtgtasksmanager' );
                return $result_array;                
            }
        ...
        if ( false === @ move_uploaded_file( $file_array['tmp_name'], $uploads['path'] . $filename ) ) {
                $result_array['outcome'] = false;
                $result_array['error'] = true;
                $result_array['message'] = __( 'Your file could not be moved into the set directory. If a file with the same name exists in that directory an overwrite may not be possible.', 'wtgtasksmanager' );
                return $result_array; 
        }    
    ...
    */
    public function file_from_url( $url, $uploads = array(), $overwrite = false, $allow = false ) {
        ...
        if( !$url_is_valid ) {
            $result['message'] = __( 'URL is invalid', 'wtgtasksmanager' );
            return false;    
        }
        ...
        if ( false !== $uploads['error'] ) {
            $result['message'] = __( 'WordPress wp_upload_dir() returned an error.', 'wtgtasksmanager' );
        ...
        if( $write_result === false ) {
            ...
            if( file_exists( $uploads['path'] . $file['originalName'] ) ) {
                $result['message'] = __( 'The filename already exists and the file cannot be overwritten.', 'wtgtasksmanager' );
                return $result;                 
            }
            
        } elseif( $write_result === false && $rename !== true ) {
            $result['failurereason'] = __( 'File already exists and renaming is not allowed.', 'wtgtasksmanager' );
            return $result;                
        }
        ...
        $message = __( 'The file was transferred.', 'wtgtasksmanager' ); 
        
        if( $overwrite_applied ) {
            $message .= ' ' . __( 'An existing file was overwritten as requested.', 'wtgtasksmanager' );
        }
        
        if( $filerename_applied ) {
            $message .= ' ' . __( 'The files name had to be changed as it already exists.', 'wtgtasksmanager' );    
        }
    ...
    */
    public function compare_csv_files( $filepath_one, $filepath_two ) {
        ...
        can switch to the newer file if required.', 'wtgtasksmanager' );      
