...
* @version 1.0
*/
class WTGTASKSMANAGER_Formbuilder extends WTGTASKSMANAGER_UI {
    ...
    public function __construct() {
        ...
        $this->WPCore = WTGTASKSMANAGER::load_class( 'WTGTASKSMANAGER_WPCore', 'class-wpcore.php', 'classes' );
        
        // get form validation array
        $this->form_val_arr = get_option( 'wtgtasksmanager_formvalidation' );
        
        // re-populate class var with the localized value
        $this->first_switch_label = __( 'Enabled', 'wtgtasksmanager' );
        $this->second_switch_label = __( 'Disabled', 'wtgtasksmanager' );
    }
    ...
    */
    public function input( $formid, $inputtype, $inputid, $inputname, $optiontitle, $titleatt = '', $required = false, $currentvalue = null, $atts_array = array(), $validation_array = array() ) {
        ...
        if( !in_array( $inputtype, $this->input_types, false ) ) {
            self::option_subline( __( 'This is a technical matter please report it. There should be a form input here that you may require before you continue.', 'wtgtasksmanager' ), __( 'Invalid Input Type', 'wtgtasksmanager' ) );
            return false;    
        }      
        ...
        if( isset( $atts_array['taxonomy'] ) ) { $this->taxonomy = $atts_array['taxonomy']; }
    ...
    */
    public function register_form( $form_id ) {
        ...
        update_option( 'wtgtasksmanager_formvalidation', $this->form_val_arr );
    }
    ...
    */
    public function deregister_form( $form_id ) {
        ...
        update_option( 'wtgtasksmanager_formvalidation', array() );
    }
        ...
        foreach( $this->form_val_arr[ $user_ID ][ $formid ] as $input_name => $input_array ) { 
            ...
            if( isset( $input_array['required']) && $input_array['required'] === true ) {
                if( $input_array['inputtype'] === 'file' ) {
                    ...
                    if( $box_checked === 0 ) {
                        $this->UI->create_notice( sprintf( __( 'Oops! You missed something. Please make selection for %s as it is a required option.', 'wtgtasksmanager' ), $input_array['optiontitle'] ), 'error', 'Small', __( 'Required Option', 'wtgtasksmanager' ) );                    
                        return false;                    
                    }                    
                    
                    // if not enough checked FAIL
                     if( $box_checked < $input_array['minimumchecks'] ) {
                        $this->UI->create_notice( sprintf( __( 'You have not checked enough boxes for the %s option.', 'wtgtasksmanager' ), $input_array['optiontitle'] ), 'error', 'Small', __( 'Check More Boxes', 'wtgtasksmanager' ) );                    
                        return false;                    
                    }
                                       
                    // if too many checked....you guessed it FAIL
                    if( $box_checked > $input_array['maximumchecks'] ) {
                        $this->UI->create_notice( sprintf( __( 'You have checked too many boxes for the %s option.', 'wtgtasksmanager' ), $input_array['optiontitle'] ), 'error', 'Small', __( 'Please Uncheck Boxes', 'wtgtasksmanager' ) );                    
                        return false;                    
                    }                    
    ...
    */
    public function apply_input_security() {
        ...
        if( !isset( $_POST['wtgtasksmanager_form_formid'] ) ) { 
            return false;
        }
        
        $form_id = $_POST['wtgtasksmanager_form_formid'];
        ...
        foreach( $_POST as $input_name => $value ) {
            ...
            if( isset( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ] ) ) {
                ...
                if( isset( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] ) && $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] === true ) {
                    ...
                    $this->UI->create_notice( __( 'Possible security violation detected. Please discontinue your action. This event has been logged for investigation.', 'wtgtasksmanager' ), 'warning', 'Small', __( 'Warning', 'wtgtasksmanager' ) );                    
                    return false;    
                }
            }    
        }                  
            ...
            foreach( $this->form_val_arr[ $user_ID ][ $form_id ] as $input_name => $input_array ) {
                ...
                if( !in_array( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['inputtype'], $this->grouped_fields ) ) {
                    ...
                    if( isset( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] ) && $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] === false ) {
                        ...
                        if( !isset( $_POST[ $input_name ] ) ) {   
                            $this->UI->create_notice( __( 'Possible security violation. Please discontinue your action. This event has been logged for investigation.', 'wtgtasksmanager' ), 'warning', 'Small', __( 'Warning', 'wtgtasksmanager' ) );                    
                            return false;                        
                        } 
                    }
                }
                ...
                if( $input_array['inputtype'] == 'menu' ) {
                    ...
                    if( $found_match === false ) {
                        $this->UI->create_notice( __( 'WTG Tasks Manager has security related concerns about a menu on the form you submitted. Please discontinue your action and contact support. This event has been logged for investigation.', 'wtgtasksmanager' ), 'warning', 'Small', __( 'Warning', 'wtgtasksmanager' ) );                    
                        return false;    
                    }
                }                
                
                // check if hacker added new checkbox
                if( $input_array['inputtype'] == 'checkboxes' ) {
                    ...
                    // if no match, the submitted item value was not registered when the form was parsed
                    if( $found_match === false ) {
                        $this->UI->create_notice( __( 'WTG Tasks Manager has found a security related problem regard the form you submitted. Please discontinue your action and contact support.', 'wtgtasksmanager' ), 'warning', 'Small', __( 'Warning', 'wtgtasksmanager' ) );                    
                        return false;    
                    }
                }
                ...
                if( $input_array['inputtype'] == 'radiogroup' ) {
                    ...
                    // if no match, the submitted item value was not registered when the form was parsed
                    if( $found_match === false ) {
                        $this->UI->create_notice( __( 'WTG Tasks Manager detected a problem and could not continue. Please try again and if it continues contact support.', 'wtgtasksmanager' ), 'warning', 'Small', __( 'Warning', 'wtgtasksmanager' ) );                    
                        return false;    
                    }
                }               
                ...
                if( $input_array['inputtype'] == 'hidden' ) {
                    ...
                    if( !isset( $_POST[ $input_name ] ) ) {
                       // arriving here indicates hidden input missing, a hack has removed it
                        $this->UI->create_notice( __( 'A security matter has arisen. Please discontinue your action and contact support if this message continues to appear. The event has been logged.', 'wtgtasksmanager' ), 'warning', 'Small', __( 'Warning', 'wtgtasksmanager' ) );                    
                        return false;                         
                    } else {
                        // if registered value does not match value submitted, hacker has changed it
                        if( $input_array['currentvalue'] !== $_POST[ $input_name ] ) {
                            $this->UI->create_notice( __( 'WTG Tasks Manager has detected a possible hack attempt. Your activity has been logged. Please discontinue your action and contact support if this message continues to appear without any known reason.', 'wtgtasksmanager' ), 'warning', 'Small', __( 'Warning', 'wtgtasksmanager' ) );                    
                            return false;                            
                        }
                    }
                }   
    ...
    */    
    public function user_can( $capability = 'activate_plugins' ){
        if(!current_user_can( $capability ) ){
            $this->UI->create_notice( __( 'Your request was rejected as your account does not have the required permission.', 'wtgtasksmanager' ), 'success', 'Small', __( 'Not Permitted', 'wtgtasksmanager' ) );          
            return false;
        }   
        return true;
    }     
    ...
    */
    public function validate_alpha( $value, $optiontitle, $atts_array ) {
        if( !ctype_alpha( $value ) ) {
            $this->UI->create_notice( sprintf( __( "The %s input is only for letters, please edit your entry and remove any numbers or special characters.", 'wtgtasksmanager' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'wtgtasksmanager' ) );               
            return false;            
        }
        return true;
    }
    ...
    */
    public function validate_alphanumeric( $value, $optiontitle ) {
        if( !ctype_alnum( $value ) ) {   
            $this->UI->create_notice( sprintf( __( "The %s input is for letters or numbers, no special characters allowed.", 'wtgtasksmanager' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'wtgtasksmanager' ) );               
            return false;
        }
        return true;
    }
    ...
    */
    public function validate_url( $url, $optiontitle ) {    
        if( !$this->PHP->validate_url( $url ) ) {
            $this->UI->create_notice(sprintf( __( "You did not enter a valid URL in the %s input. Please ensure that all characters match the online URL.", 'wtgtasksmanager' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'wtgtasksmanager' ) );               
            return false;
        }        
        return true;
    } 
    ...
    */
    public function validate_numeric( $value, $optiontitle ) {  
        if( !ctype_digit( $value ) ) {
            $this->UI->create_notice( sprintf( __( "Only numbers are meant to be entered into the %s field.", 'wtgtasksmanager' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'wtgtasksmanager' ) );               
            return false;
        }        
        return true;    
    }
    ...
    */
    public function validate_maxlength( $str, $optiontitle, $atts_mixed ) {
        if ( strlen( $str ) > $atts_mixed ) {
            $this->UI->create_notice( sprintf( __( "The maximum length for the %s field is %s ", 'wtgtasksmanager' ), $optiontitle, $atts_mixed ), 'error', 'Small', __( 'Too Long', 'wtgtasksmanager' ) );               
            return false;
        }        
        return true;        
    }    
    ...
    */
    public function validate_minlength( $str, $optiontitle, $atts_mixed ) {
        if ( strlen( $str ) < $atts_mixed ) {
            $this->UI->create_notice( sprintf( __( "The minimum length for the %s field is %s ", 'wtgtasksmanager' ), $optiontitle, $atts_mixed ), 'error', 'Small', __( 'Too Short', 'wtgtasksmanager' ) );               
            return false;
        }        
        return true;        
    }
    ...
    */
    public function validate_regex( $str, $optiontitle, $atts_mixed ) {
        if ( !preg_match( $atts_mixed[0], $str ) ) {
            $this->UI->create_notice( sprintf( __( "The %s field entry is not the correct format. A specific pattern of characters is required i.e. %s.", 'wtgtasksmanager' ), $optiontitle, $atts_mixed[1] ), 'error', 'Small', __( 'Invalid Value', 'wtgtasksmanager' ) );               
            return false;
        }        
        return true;    
    }
    ...
    /**
    * validate a numeric comma string i.e  32,343,66,77,99
    ...
    * @version 1.0
    */
    public function validate_numericlist( $str, $optiontitle ) {
        if ( !preg_match( '[0-9]+(,[0-9]+)*', $str ) ) {
            $this->UI->create_notice( sprintf( __( "The %s field entry is not the correct format. A specific pattern of characters is required i.e. %s.", 'wtgtasksmanager' ), $optiontitle ), 'error', 'Small', __( 'Invalid Value', 'wtgtasksmanager' ) );               
            return false;
        }        
        return true;    
    }
    ...
    */
    public function input_switch(){
        ...
        <!-- Option Start -->
        <tr valign="top">
            <th scope="row"><?php _e( $this->optiontitle, 'wtgtasksmanager' ); ?></th>
            <td>
    ...
    public function input_checkboxes(){
        ...
        <tr valign="top">
            ...
            foreach( $this->itemsarray as $item_value => $item_title ) {
                ...
                echo '<label for="' . $this->inputid . $i . '"><input type="checkbox" name="' . $this->inputid . $i . '" id="' . $this->inputid . $i . '" value="' . $item_value . '"' .  $checked . $disabled . '>' . $item_title . '</label>';
                $first = false;
                ++$i;
            }
            ?>
        ...
        <tr valign="top">
            ...
            <td>                                     
                <select name="<?php echo $this->inputname;?>" id="<?php echo $this->inputid;?>"<?php if( $this->disabled ){ echo ' disabled';} ?>>
                    ...
                    <option value="notselected" <?php echo $selected;?>><?php _e( 'None Selected', 'wtgtasksmanager' ); ?></option> 
    ...
    */
    public function input_menu_cronrepeat() {
        ...
        <tr valign="top">
            ...
            <td>            
                <select name="<?php echo $this->inputname;?>" id="<?php echo $this->inputid;?>"<?php if( $this->disabled ){ echo ' disabled';} ?>>
                    <option value="hourly" <?php if( $this->currentvalue == 'hourly' ){ echo $selected; }?>><?php _e( 'Hourly', 'wtgtasksmanager' ); ?></option>
                    <option value="twicedaily" <?php if( $this->currentvalue == 'twicedaily' ){ echo $selected; }?>><?php _e( 'Twice Daily', 'wtgtasksmanager' ); ?></option>
                    <option value="daily" <?php if( $this->currentvalue == 'daily' ){ echo $selected; }?>><?php _e( 'Daily', 'wtgtasksmanager' ); ?></option>
                </select>                  
            </td>
        </tr>
        <!-- Option End --><?php     
    ...
    */
    ...
    /**
    * A menu of taxonomy terms for the giving post and giving taxonomy (one taxonomy)
    * 
    * This is for none heirarchical taxonomies.
    * 
    * A single taxonomy (will need to be adapted or another function for multiple taxonomies in a single menu)
    * 
    * @author Ryan R. Bayne
    * @package WTG Tasks Manager
    * @since 0.0.1
    ...
    */
    public function input_taxonomyheir(){ 
        $args = array(
            'orderby'           => 'name', 
            'order'             => 'ASC',
            'hide_empty'        => false, 
        ...
        $terms = get_terms( array( $this->taxonomy ), $args);     
        ?>
        <!-- Option Start -->        
        <tr valign="top">
            <th scope="row"><label for="<?php echo $this->inputid; ?>"><?php echo $this->optiontitle; ?></label></th>
            <td>            
                <select name="<?php echo $this->inputname;?>" id="<?php echo $this->inputid;?>"<?php if( isset( $this->disabled ) && $this->disabled ){ echo ' disabled';} ?>>
                    <?php            
                    foreach( $terms as $array_key => $term ){
                        $selected = '';
                        if( $term->term_id == $this->currentvalue ){
                            $selected = 'selected="selected"';
                        } 
                        echo '<option value="' . $term->term_id . '" ' . $selected . '>' . $term->name . ' (' . $term->term_id . ')</option>';    
                    }
                    ?>
                </select>                  
            </td>
    ...
    */
    public function hidden_basic( $form_id, $id, $name, $current_value ) {
        self::input( $form_id, 'hidden', $id, $name, __( 'Hidden Input ' . $id, 'wtgtasksmanager' ), __( 'Hidden Input ' . $id, 'wtgtasksmanager' ), true, $current_value, array(), array() ); 
    }
    ...
    * @version 1.0
    */
    public function checkboxes_basic( $form_id, $id, $name, $title, $items_array, $current_value, $required = false, $validation_array = array() ) {
        self::input( $form_id, 'checkboxes', $id, $name, $title, $title, $required, $current_value, array( 'itemsarray' => $items_array ), $validation_array );        
    }
    ...
    * @version 1.0
    */
    public function checkboxes_advanced( $form_id, $id, $name, $title, $current_value, $items, $minimumchecks = 1, $maximumchecks = 100, $disabled = false, $required = false, $tabled = true, $html5 = true, $ajax = true ) {                    
        self::input( $form_id, 'checkboxes', $id, $name, $title, $title, $required, $current_value, array( 'itemsarray' => $items, 'disabled' => $disabled, 'tabled' => $tabled, 'html5' => $html5, 'ajax' => $ajax, 'minimumchecks' => $minimumchecks, 'maximumchecks' => $maximumchecks ) );            
    }
