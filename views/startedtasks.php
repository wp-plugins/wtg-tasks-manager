 ...
 * @since 0.0.1
 */
class WTGTASKSMANAGER_Startedtasks_View extends WTGTASKSMANAGER_View {
    ...
    protected $view_name = 'startedtasks';
    ...
    */
    public function datatables( $data, $box ) {    
        $WPTableObject = new WTGTASKSMANAGER_WPTable_StartedTasks();   
        ?>
        
        <form method="get">
            <input type="hidden" name="page" value="<?php echo $_REQUEST['page']; ?>" />
            <?php
            $WPTableObject->prepare_items_further( false, 20 );
    ...
    * @version 1.0
    */
    public function postbox_startedtasks_example( $data, $box ) {    
...
* @version 1.0
*/
class WTGTASKSMANAGER_WPTable_StartedTasks extends WP_List_Table {
    ...
    function column_default( $item, $column_name ){   
        ...
        switch( $column_name){
            ...
            case 'wtgtaskprogress':
                return get_post_meta( $item->ID, 'wtgprogress', true );
                break;            
            case 'wtgtaskstarter':
                $user_id = get_post_meta( $item->ID, 'wtgtaskstarter', true );
                if( is_numeric( $user_id ) ) {
                    $user_data = get_userdata( $user_id );    
                    return $user_data->user_login;
                }
                
                return __( 'Unknown', 'wtgtasksmanager' );
    ...
    **************************************************************************/
    ...
    function get_columns() {
        $columns = array(
            ...
            'wtgtaskstarter' => __( 'Started By', 'wtgtasksmanager' ),
